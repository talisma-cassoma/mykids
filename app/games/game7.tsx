...
function createRandomMainQueue() {
  return shuffle(allQuestions).slice(0, 20);
}

function chooseQuestionFromQueues(mainQueue: Question[], mistakeQueue: Question[]) {
  const hasMain = mainQueue.length > 0;
  const hasMistakes = mistakeQueue.length > 0;
  ...
  const sourceQueue = useMainQueue ? mainQueue : mistakeQueue;
...
export default function MultiplicationGameScreen() {
  ...
  const [mainQueue, setMainQueue] = useState<Question[]>(() => createRandomMainQueue());
  ...
  const generateNextQuestion = useCallback(() => {
    const selected = chooseQuestionFromQueues(mainQueue, mistakeQueue);

    if (!selected) {
      const replacementQueue = createRandomMainQueue();
      setMainQueue(replacementQueue);
    ...
    if (source === "main") {
      setMainQueue((prev) => prev.filter((_, i) => i !== index));
    } else {
      setMistakeQueue((prev) => prev.filter((_, i) => i !== index));
    }
    ...
    setAnswerLocked(false);
    resetTimer();
  }, [mainQueue, mistakeQueue, resetTimer]);
  ...
  const handleAnswer = useCallback((value: number) => {
    ...
    if (isCorrect) {
      ...
      setMainQueue((prev) => prev.filter((item) => item.id !== question.id));
      setMistakeQueue((prev) => prev.filter((item) => item.id !== question.id));
      setScore(nextScore);
    ...
    setStatusText("Oops!");
    setMainQueue((prev) => prev.filter((item) => item.id !== question.id));
    setMistakeQueue((prev) => [...prev, question]);
